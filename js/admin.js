...
(function () {
  ...
  function gcalLink(f) {
    ...
    var cim = [f.irsz, f.telepules].filter(Boolean).join(" ") + (f.cim ? (", " + f.cim) : "") + (f.emelet ? (", " + f.emelet + ". em.") : "") + (f.ajto ? (" " + f.ajto + ". ajtó") : "");
    var reszletek = [
      "Ügyfél: " + (f.nev_cegnev || ""),
      "Telefon: " + (f.telefon || ""),
      "E-mail: " + (f.email || ""),
      ...
      .then(function (res) {
        ...
        el.innerHTML = res.data.map(function (f) {
          ...
          return '<article class="admin-foglalas">'
            ...
            + '<dt>Cím</dt><dd>' + esc([f.irsz, f.telepules].filter(Boolean).join(" ")) + (f.cim ? (", " + esc(f.cim)) : "") + (f.emelet ? (", " + esc(f.emelet) + ". em.") : "") + (f.ajto ? (" " + esc(f.ajto) + ". ajtó") : "") + '</dd>'
